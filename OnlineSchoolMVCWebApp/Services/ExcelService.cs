...
namespace OnlineSchoolMVCWebApp.Services
{
    public class ExcelService
    {
                    ...
                    using (XLWorkbook workBook = new XLWorkbook(stream))
                    {
                        foreach (IXLWorksheet worksheet in workBook.Worksheets)
                        {
                            ...
                            foreach (IXLRow row in worksheet.RowsUsed().Skip(1))
                            {
                                try
                                {
                                    await CreateCource(row, author, newCategory);
                                }
                                catch (Exception e)
                                {
            ...
            using (XLWorkbook workbook = new XLWorkbook())
            {
                ...
                foreach (var subjectCategory in subjectCategories)
                {
                    if (cources.Select(c => c.SubjectCategoryId).Contains(subjectCategory.Id))
                    {
                        ...
                        await HeaderGenerate(worksheet);
                        await ContentGenerate(worksheet, cources, subjectCategory);
                    }
                }
        ...
        private async Task CreateCource(IXLRow row, Author author, SubjectCategory subjectCategory)
        {
            Cource cource = new Cource();
            cource.Title = row.Cell(1).Value.ToString();
            cource.Description = row.Cell(3).Value.ToString();
            cource.SubjectCategory = subjectCategory;
        ...
        private async Task HeaderGenerate(IXLWorksheet worksheet)
        {
        ...
        private async Task ContentGenerate(IXLWorksheet worksheet, List<Cource> cources, SubjectCategory subjectCategory)
        {
            int index = 0;
